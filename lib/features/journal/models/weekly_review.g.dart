...
WeeklyReview _weeklyReviewDeserialize(
  ...
  object.aiInsights = reader.readString(offsets[0]);
  object.avgEnergy = reader.readDouble(offsets[1]);
  object.createdAt = reader.readDateTime(offsets[2]);
  object.id = id;
  object.totalFocusMinutes = reader.readLong(offsets[3]);
  object.weekStartDate = reader.readDateTime(offsets[4]);
  return object;
}
...
extension WeeklyReviewByIndex on IsarCollection<WeeklyReview> {
  Future<WeeklyReview?> getByWeekStartDate(DateTime weekStartDate) {
    return getByIndex(r'weekStartDate', [weekStartDate]);
  }

  WeeklyReview? getByWeekStartDateSync(DateTime weekStartDate) {
  ...
  Future<List<WeeklyReview?>> getAllByWeekStartDate(
  ...
  List<WeeklyReview?> getAllByWeekStartDateSync(
      List<DateTime> weekStartDateValues) {
    final values = weekStartDateValues.map((e) => [e]).toList();
  ...
  Future<Id> putByWeekStartDate(WeeklyReview object) {
    return putByIndex(r'weekStartDate', object);
  }

  Id putByWeekStartDateSync(WeeklyReview object, {bool saveLinks = true}) {
    return putByIndexSync(r'weekStartDate', object, saveLinks: saveLinks);
  }

  Future<List<Id>> putAllByWeekStartDate(List<WeeklyReview> objects) {
    return putAllByIndex(r'weekStartDate', objects);
  }

  List<Id> putAllByWeekStartDateSync(List<WeeklyReview> objects,
      {bool saveLinks = true}) {
